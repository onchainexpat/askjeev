  ...
  <div style="margin:10px 0;">
    ...
    <div id="self-verified-section" style="margin-top:10px;">
      ...
      <div style="display:flex;gap:6px;flex-wrap:wrap;margin-bottom:8px;">
        <button id="btn-free-ask" onclick="selfFreeCall('/api/demo/self-ask','POST',{prompt:document.getElementById('self-prompt').value||'What is cross-chain arbitrage?'})" title="Free inference via Bankr LLM — verify with Self to unlock." class="demo-btn-gold">Ask Bankr (verify to unlock)</button>
        <button id="btn-free-analyze" onclick="selfFreeCall('/api/demo/self-analyze','POST',{prompt:document.getElementById('self-prompt').value||'Analyze the current ETH market in one paragraph.'})" title="Free private analysis via Venice AI — verify with Self to unlock." class="demo-btn-gold">Private Analyze (verify to unlock)</button>
      ...
      var pollInterval = setInterval(async function() {
        ...
        try {
          ...
          if (sd.verified) {
            ...
            // Unlock self-verified free buttons — add glow and update labels
...
"Bridge 1 USDC from base to arbitrum via
 https://synthesis-hackathon-beta.vercel.app/api/bridge"</pre>
  ...
  // Serve logo
  app.get('/logo-askjeeve.png', async (c) => {
    try {
      const fs = await import('fs');
      const path = await import('path');
      const imgPath = path.resolve(process.cwd(), 'public', 'logo-askjeeve.png');
  ...
  app.post('/api/demo/self-rebalance', async (c) => {
    try {
      ...
      // Step 1: Read full portfolio via Zerion API (all chains, all tokens)
      ...
      const positions = (zerionData.data || []).slice(0, 20); // Top 20 by value
      ...
      for (const p of positions) {
        ...
        if (value > 1) {
          ...
          holdings.push({
            token: name,
            symbol,
            balance: qty > 1000000 ? Math.round(qty).toLocaleString() : qty.toFixed(4),
      ...
      for (const h of holdings) {
        h.percent = totalUsd > 0 ? Math.round(h.valueUSD / totalUsd * 100) : 0;
      }

      const portfolio = {
        address: targetAddress,
        source: 'Zerion API (all chains)',
      ...
      // Step 4: Generate actionable swap routes
      const routes: any[] = [];

      // Find memecoins/small tokens to suggest swapping
      ...
      if (riskStrategy === 'conservative') {
        // Suggest selling top memecoins → ETH or USDC
        for (const meme of memeHoldings.slice(0, 3)) {
          routes.push({
            action: `Sell ${meme.symbol} ($${meme.valueUSD}) → USDC via Uniswap`,
            token: meme.symbol,
            currentValue: '$' + meme.valueUSD,
            reason: 'High volatility memecoin — convert to stablecoin for capital preservation',
            route: 'Uniswap (Ethereum L1) or bridge to Base first',
          });
        }
        if (ethHolding && ethHolding.valueUSD > 1000) {
          routes.push({
            action: `Swap 30% ETH ($${Math.round(ethHolding.valueUSD * 0.3)}) → USDC`,
            reason: 'Reduce ETH exposure for stability',
            route: 'Uniswap (Ethereum L1 or Base)',
          });
        }
      } else {
        // Aggressive: suggest buying more ETH with stablecoins
        if (usdcHolding && usdcHolding.valueUSD > 100) {
          routes.push({
            action: `Swap 70% USDC ($${Math.round(usdcHolding.valueUSD * 0.7)}) → ETH`,
            reason: 'Maximize ETH exposure for growth',
            route: 'Uniswap (Ethereum L1 or Base)',
          });
        }
        // Keep top memecoins, suggest consolidating small ones
        const smallMemes = memeHoldings.filter(m => m.valueUSD < 1000);
        if (smallMemes.length > 0) {
          routes.push({
            action: `Consolidate ${smallMemes.length} small memecoin positions ($${Math.round(smallMemes.reduce((s, m) => s + m.valueUSD, 0))}) → ETH`,
            tokens: smallMemes.map(m => m.symbol).join(', '),
            reason: 'Small positions have high gas-to-value ratio — consolidate into ETH',
            route: 'Uniswap batch swaps on Ethereum L1',
          });
        }
      }

      // Always suggest cross-chain bridge
      routes.push({
        action: 'Bridge USDC to L2 (Base/Arbitrum) for lower gas',
        bridge: 'Across Protocol (ERC-7683)',
        reason: 'L2s have 100x lower gas costs — move stablecoins to Base or Arbitrum',
      });
